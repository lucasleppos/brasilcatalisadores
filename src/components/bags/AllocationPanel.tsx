...
export function AllocationPanel({ bags, onAllocated }: AllocationPanelProps) {
  ...
  const loadAvailableMaterials = async () => {
    ...
    // Peça em Sacola → agrega as peças conferidas por marcação Flex / Carbono (2 linhas por OP)
    ...
    sacolaAggIds.forEach(pid => {
      ...
      (["flex", "carbono"] as const).forEach(kind => {
        ...
        kindItems.forEach((item: any) => {
          const result = item.calc_result as any;
          const input = item.calc_input as any;
          const w = netWeightOf(item) || (result?.netWeightKg || 0);
          weight += w;
        ...
        available.push({
          ...
          supplierName: purchase.supplier_name,
          supplierBranch: branchMap.get(purchase.supplier_id) || "",
          ptPpm: weight > 0 ? ptW / weight : 0,
          pdPpm: weight > 0 ? pdW / weight : 0,
          rhPpm: weight > 0 ? rhW / weight : 0,
