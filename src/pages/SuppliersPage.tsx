...
export default function SuppliersPage() {
  ...
  const canDelete = canDo("fornecedores", "delete");
  const canImport = canDo("fornecedores", "import");
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {!hideDocument && <SortableTableHead column="document" currentColumn={sort.column} direction={sort.direction} onToggle={toggleSort}>CNPJ/CPF</SortableTableHead>}
              ...
              {sorted.length === 0 ? (
                ...
                sorted.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    {!hideDocument && <TableCell className="text-sm">{s.document}</TableCell>}
      ...
      {canImport && <SupplierImport open={importOpen} onOpenChange={setImportOpen} onImport={handleImport} />}
    </div>
  );
}
